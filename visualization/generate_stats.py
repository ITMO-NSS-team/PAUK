...
The regex checks fire on 0.6% and every hit is genuinely malformed.
...
from checks import CHECKS, BY_ID
...
def collect(drv):
    ...
    for c in CHECKS:
        ...
        denom = scalar(drv, c["of"]) if c.get("of") else None
        checks.append({
            ...
            "hint": c.get("hint"),
            "has_examples": bool(c.get("examples")),
        })
