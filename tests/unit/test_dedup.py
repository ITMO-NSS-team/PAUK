...
class DedupStageTest(unittest.TestCase):
    def run_stage(self, people, publications=(), catalog_rows=None):
        ...
        # An explicit data_dir keeps the stage off the machine's own staff
        # catalog: without one it would read whatever data/static holds.
        ...
        result = DedupStage(self.prepared, self.raw, config).run()
