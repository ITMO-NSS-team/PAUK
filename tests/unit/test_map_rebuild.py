...
def snapshot() -> dict[str, list]:
    ...
    Nodes come back keyed by column name, relationships as positional tuples
    — mixing the two up is the kind of thing that only shows when the map
    comes out empty.
    """
    return {
        "persons": [
        ...
        "publications": [
            {"id": "W1", "title": "Статья про графы", "journal": "Журнал", "doi": "10.1000/x",
             "publication_date": "2024-05-01", "year": 2024, "has_code": True,
             "code_url": "https://github.com/org/repo", "fields": ["Computer Science"]},
        ],
        "repositories": [
            {"id": "R1", "name": "repo", "url": "https://github.com/org/repo",
             "description": "описание", "stars_num": 42, "owner": "octocat",
             "owner_type": "Organization", "language": "Python", "topics": ["graphs"],
             "last_updated": "2024-05-01", "license": "MIT", "archived": False,
             "forks_num": 3, "is_fork": False},
        ],
...
class EmptyGraphTest(unittest.TestCase):
    ...
    def test_a_publication_with_no_itmo_author_builds(self):
        db = self.blank()
        db["publications"] = [{"id": "W1", "title": "Статья", "journal": None, "doi": None,
