 ...
 * `search.js::runSearch()` (`targetTab = repo ? 2 : pub ? 3 : 1`).
 * Департамента здесь нет: у него нет своей вкладки с графом, dept-хиты
 * вкладку не меняют (см. клик по dept-результату ниже).
 */
const HIT_KIND_TAB: Record<"author" | "repo" | "pub", TabId> = { author: 1, repo: 2, pub: 3 };

/**
 * Вкладка "Поиск" — единый поиск по всем авторам, репозиториям,
 * публикациям и департаментам сразу, в отличие от вкладок 1-3, которые
 * показывают только один вид узлов. Устройство похоже на остальные
 * вкладки (`renderList` + `store.selection`), но список строится не из
 * `data` напрямую, а из текстового запроса пользователя — поэтому здесь
 * есть собственное локальное состояние (`query`), которое не кладём в
 * общий Store: это состояние поля ввода одной конкретной вкладки, а не
 * что-то, на что должны реагировать другие части приложения.
 *
 * Клик по результату-автору/репозиторию/публикации переключает активную
 * вкладку на ту, что реально рисует граф для этого вида узла (см.
 * {@link HIT_KIND_TAB}) — иначе выбор был бы невидим на карте, которая
 * рисует граф только активной вкладки, а у вкладки "Поиск" своего графа
 * нет. Клик по результату-департаменту вкладку не меняет: у департамента
 * нет собственных координат и собственной вкладки-графа.
 *
...
export const searchTab: TabModule = {
  mount(container, store, map, data, searchDetails) {
    ...
    /**
     * Перерисовывает список результатов под текущий `query` (локальная
     * переменная замыкания, не Store) и язык.
     *
     * @param lang - язык интерфейса, влияет на подпись вида результата (`kindLabel`).
     */
    ...
    /**
     * Переключает язык вкладки: обновляет placeholder поля ввода и
     * перерисовывает результаты (индекс поиска к этому моменту уже
     * пересобран под новый язык, если он реально изменился — см. подписку ниже).
     *
