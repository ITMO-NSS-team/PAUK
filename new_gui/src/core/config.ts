...
export const MAP_CONFIG = {
  ...
  edge: {
    ...
    // способа "прятать рёбра при отдалении", как есть для подписей. 0.2 —
    // рёбра появляются только при заметном приближении (прямая просьба),
    // не на обзорном зуме всего графа.
    visibleBelowRatio: 0.4,
  },
} as const;
