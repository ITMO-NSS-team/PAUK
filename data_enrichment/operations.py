...
class EnrichDepartments(PerRecordOperation):
    ...
    def pending(self, ctx) -> list:
        # '-' не вечная метка: такие персоны пересматриваются на следующем прогоне.
...
class TranslateNames(PerRecordOperation):
    ...
    def pending(self, ctx) -> list:
        # Берём и NULL, и пустую строку: иначе неудачный первый прогон
        # не пересмотрится никогда.
...
class MatchGithub(WholeSetOperation):
    ...
    def process_all(self, ctx) -> int:
        ...
        # Фаза 2: одна точка записи — всё через единственный коннектор.
