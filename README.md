...
Ссылки на код извлекаются из абстракта OpenAlex и, если у публикации есть
`pdf_url`, из полного текста PDF постранично (`pauk enrich code_links`,
кэш PDF — `data/pdf/<group>/`) — как из видимого текста (включая голые
упоминания вида `github.com/org/repo` без `https://`), так и из настоящих
PDF-гиперссылок, чей видимый текст сам URL не показывает. Если `pdf_url`
нет, но задан `PAUK_PDF_CRAWLER_URL` (см. `.env.example`), PDF ищется по
DOI через [PDF-Crawler-Service](https://github.com/gurinboru/PDF-Crawler-Service).
Извлечённый текст сохраняется в `Publication.full_text`.

`context`/`page_number` у `MENTIONS_LINK` — список вхождений на публикацию
(одна ссылка может встречаться и в абстракте, и на нескольких страницах).
Перенос слова/ссылки через дефис на границе строки PDF распознаётся и
склеивается; перенос без дефиса — принципиально нет (нечем отличить от
границы предложения, попытка это сделать однажды слепила соседний текст
внутрь ссылки). Если PDF найден, но скачать/распарсить не удалось (403,
таймаут), этап помечается `failed` и ретраится при следующем прогоне, но
результат по абстракту всё равно сохраняется.
