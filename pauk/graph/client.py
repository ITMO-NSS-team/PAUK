...
class Neo4jClient:
    ...
    def upsert_nodes_batch(self, labels: str | list[str], nodes: list[tuple[str, dict]]):
        """
            .
        :param labels:   ("Person")    (["Person", "Itmo"]).
        :param nodes:   (node_id, properties).
        """
        if not nodes:
            return
    ...
    def upsert_relationships_batch(
        ...
        """
            .
        :param relationships:   (src_id, tgt_id, rel_properties).
        :param tgt_match_prop: ,      —
              "id" (. Repository   "url", GitHubProfile
             "login").
        :return:    / —  
            len(relationships),      ( 
            ,  ).
