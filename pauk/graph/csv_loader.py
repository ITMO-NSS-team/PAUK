"""Загрузка узлов/связей из общих CSV (формат id/labels/properties,
start_id/end_id/src_label/tgt_label/type/properties) — прямой перенос из
старого scripts/data_to_graph.py, поведение не менялось, только импорт стал
относительным (было: `from neo4j_client import ...` — падало в обоих режимах
запуска, см. neo4j-connector.md в Obsidian).
"""
...
def load_nodes_from_csv(client: Neo4jClient, csv_path: str, batch_size: int = CHUNK_SIZE):
    """Столбцы: id, labels, properties (properties — JSON-строка)."""
...
def load_csv_dir(client: Neo4jClient, data_dir: Path) -> None:
    """Узлы (*_nodes.csv), потом связи (*_rels.csv) — тот же порядок, что и
    у jsonl_loader.load_jsonl_dir."""
    if not data_dir.exists():
        logger.error("Директория с данными не найдена: %s", data_dir)
        return

    node_files = discover_files(data_dir, "_nodes.csv")
    rel_files = discover_files(data_dir, "_rels.csv")
    logger.info("Найдено файлов узлов: %d, файлов связей: %d", len(node_files), len(rel_files))

    for file_path in node_files:
        logger.info("Узлы: %s", file_path.name)
        try:
            load_nodes_from_csv(client, str(file_path))
        except Exception:
            logger.error("Ошибка при загрузке файла узлов %s", file_path.name, exc_info=True)

    for file_path in rel_files:
        logger.info("Связи: %s", file_path.name)
        try:
            load_relationships_from_csv(client, str(file_path))
        except Exception:
            logger.error("Ошибка при загрузке файла связей %s", file_path.name, exc_info=True)
