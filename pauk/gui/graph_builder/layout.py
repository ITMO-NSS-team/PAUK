...
# latitude] by pauk/gui/web/src/map/build.ts::toLngLat() for MapLibre. 30/970
...
def fit_coords(pos: Mapping[str, Sequence[float]]) -> dict[str, tuple[float, float]]:
    ...
    Args:
        pos: Node positions from `fa2_layout` (numpy arrays `[x, y]` or
            plain tuples `(x, y)`) - either works, the function
...
def fa2_layout(graph: nx.Graph, max_iter: int, seed: int) -> dict[str, tuple[float, float]]:
    """ForceAtlas2 with the Barnes-Hut approximation (`fa2_modified`):
    O(n log n) per iteration, unlike networkx's `forceatlas2_layout`, which
    builds a dense n x n distance matrix and runs out of memory past ~15k nodes.

    Args:
        graph: Graph with a `weight` edge attribute.
        max_iter: Iteration count.
        seed: Seed for the start positions - the algorithm has no other
            randomness, so the same seed gives the same layout.
    ...
    return ForceAtlas2(verbose=False).forceatlas2_networkx_layout(
        graph, pos=start, iterations=max_iter, weight_attr="weight"
    )
...
def coauthor_pairs(authors: list[str], external_ids: frozenset[str]) -> Iterable[tuple[str, str]]:
    """Coauthor pairs of one publication, each as `(smaller, larger)`.

    Every pair for a normal publication; for a large collaboration paper
    (more than `EDGE_THRESHOLDS.hyperauthor_pub_size` authors, up to ~1700
    in the real data) only pairs with at least one ITMO author - all pairs
    there would be millions of edges between external people who never
    worked together in any real sense.

    Args:
        authors: Sorted unique author ids of the publication.
        external_ids: Authors that are not ITMO.
    """
    if len(authors) <= EDGE_THRESHOLDS.hyperauthor_pub_size:
        return combinations(authors, 2)
    itmo = [a for a in authors if a not in external_ids]
    return {(a, b) if a < b else (b, a) for a in itmo for b in authors if a != b}
...
def fa2_blended_layout(
    ...
    if giant:
        ...
        pos = fit_coords(fa2_layout(sub, max_iter, seed))
...
class GraphLayoutBuilder:
    ...
    def build(self, seed: int) -> Layout:
        ...
        for _pid, pers in self.authorship.pub_authors.items():
            for pair in coauthor_pairs(sorted(set(pers)), self.authorship.external_ids):
                coauth[pair] += 1
