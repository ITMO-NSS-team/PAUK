...
- Пять мест изнутри не останавливаются и доходят до своего конца: этапы
  `emails`, `repositories`, `dedup`, `github_match` и пакетный проход
  `persons` по OpenReview. Отмена проверяется там, где этап сообщает, как
  далеко продвинулся, а эти пять через общий счётчик не ходят. Отмену они
  заметят на выходе.
