...
Журнал правок сам не подрезается. Раз в полгода его стоит укорачивать:

```bash
uv run pauk admin audit trim            # посчитает, сколько уйдёт
uv run pauk admin audit trim --apply    # уберёт
```
