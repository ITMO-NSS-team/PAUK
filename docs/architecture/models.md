...
- **`Person`** — id = голый OpenAlex author ID (один человек — один узел,
  ...
  лейбл `:Itmo` «липкий»). Русские ФИО (`first_name_ru`/`second_name_ru`/
  `surname_ru`) — поля в модели, источник для них в пайплайне не
  подключён. `github`/`google_scholar`/`openreview` заполняются через
  OpenReview-профиль, найденный по совпадению ORCID/почты.
- Блок под комментарием `# stub` (`scopus_id`, `researcher_id`, `h_index`,
  `linkedin`, `wikipedia` и ещё полтора десятка полей) — поля из
  предложенной Камилем схемы графа, ни один pipeline stage их не
  заполняет. Оставлены намеренно, чтобы модель/коннектор/визуализация уже
...
- **`GitHubProfile`** — аккаунт-владелец репозитория (`login` уникален,
  отдельно от `id`).
