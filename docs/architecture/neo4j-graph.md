...
**Связи.** Для рёбер записывается только удаление (`kind: "rel"`,
`op: "delete"`): добавленное руками ребро публикацию и так переживает —
загрузчик рёбра создаёт, но никогда не удаляет те, о которых не знает.
А удалённое пересоздаётся `MERGE` из той же prepared-строки, поэтому
