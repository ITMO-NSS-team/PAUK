...
Дедуп **департаментов** — отдельная команда `pauk dedup departments` с
собственной воронкой (нормализация → блокинг → сигналы → LLM-арбитраж),
описана в [../graph-dept-dedup.md](../graph-dept-dedup.md).
