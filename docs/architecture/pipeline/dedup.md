...
### Экспериментальный resolver

`graph/person_resolution.py` содержит изолированную версию нового алгоритма.
Она пока не подключена к записи в Neo4j: это позволяет отдельно ревьюить и
воспроизводить решение для пары, не меняя текущий дедуп. В модуле зафиксированы
коэффициенты LogReg, точные prompt-контракты обоих Qwen-этапов, сериализация
доказательств и конечный автомат решений.
