...
сущность — персону, публикацию, репозиторий. Последний этап в `ALL_STAGES`
— работает над результатом всех предыдущих.
...
| Источник строк | JSONL с диска | Cypher-запросы к графу |
| Куда пишет | JSONL + журнал в группе | Neo4j + журнал в `data/cache/` |
| Зачем нужен отдельно | ловит дубли внутри одного сбора | ловит дубли, чьи записи попали из **разных** прогонов и никогда не лежали рядом на диске |
...
`openalex_works.jsonl` (сырые ответы) остаётся верным по-записно источником.
