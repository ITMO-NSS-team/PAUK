...
Обычные найденные ссылки `code_links` оставляет без вердикта. Следующий
...
- `is_relevant=null` — контекста недостаточно либо запрос/ответ модели
  завершился ошибкой.

Явное неопределённое решение хранит `llm_confidence` и `llm_reason`; ошибка
очищает прежний вердикт и оставляет стейдж в `FAILED`, чтобы следующий запуск
повторил запрос. Архивный репозиторий Zenodo — единственное детерминированное
исключение: `true`, уверенность `1.0`, причина
...
исторический контракт с GUI. Сторонние и неопределённые ссылки в список
не попадают. Если хотя бы один запрос к модели завершился ошибкой,
