...
`merge` удаляет дубль вместе со связями, и из графа это не отменить.
Для `Person` откат всё же есть, но в другом месте и на других условиях:
очередь «Спорные случаи» в панели собирает запись заново из её
prepared-строки (`pauk/graph/unmerge.py`), пока эта строка существует.
Для `Publication` и `Repository` обратного хода нет. Команда спрашивает
подтверждение, `--yes` его пропускает.
...
растут и ничем не подрезаются. Команда убирает из обеих то, что старше
